function [ID_RESULTS_PATH]=ID_app_treadmill(MOT_FILE,TRC_FILE, path, IK_results_path, leg, FP_used, setupIDname)
% tic
...
if strcmp(leg, 'Right') && FP_used==1
    external_loads = ExternalLoads([cd, '/', 'ExternalForce_Setup_FP1_Right_Treatmill.xml'],1); %this is the stupid fore file
elseif strcmp(leg, 'Left') && FP_used==1
    external_loads = ExternalLoads([cd, '/', 'ExternalForce_Setup_FP1_Left_Treatmill.xml'],1); %this is the stupid fore file
end
...
extLoadsName = ['Setup_force_',trialname, '_', leg,'.xml'];
lowpassfilter= 6;
